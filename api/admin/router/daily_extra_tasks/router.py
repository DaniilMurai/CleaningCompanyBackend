...
async def get_daily_extra_tasks(
        params: Annotated[schemas.DailyExtraTaskUpdate, Query()],
        service: AdminDailyExtraTaskService = Depends()
) -> list[schemas.DailyExtraTaskResponse]:
    return await service.get_daily_extra_tasks(params)
...
async def create_daily_extra_task(
    ...
    return await service.create_daily_extra_task(data)
...
async def edit_daily_extra_task(
    ...
    return await service.update_daily_extra_task(daily_extra_task_id, data)
...
async def delete_daily_extra_task(
    ...
    return await service.delete_daily_extra_task(daily_extra_task_id)
